...
export default async function getApplicantResult() {
  try {
    const sql = `SELECT applicant_jury_mapping.id AS id,applicant_jury_mapping.score AS score, applicant_jury_mapping.jury_id, applicant_jury_mapping.comment, applicant_jury_mapping.review_status, applicants.name as applicantName,applicants.phone as phone, applicants.email as email, jury_details.name as juryName  FROM applicant_jury_mapping 
